...
const MessageItem: React.FC<MessageItemProps> = ({ message, isOwn }) => {
  ...
  return (
    <div className={`flex w-full ${isOwn ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-1 duration-300`}>
      <div className={`max-w-[80%] md:max-w-[65%] flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
        
        {/* Nome do Usuário (Apenas para o lado esquerdo/outros) */}
        {!isOwn && (
        ...
        `}>
          ...
          <div className={`flex items-center gap-1.5 mt-1.5 justify-end ${isOwn ? 'text-indigo-100/70' : 'text-zinc-400 dark:text-zinc-500'}`}>
